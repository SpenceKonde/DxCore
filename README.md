...
Like most recent parts, the Dx-series parts have multiple pin-mapping options for many of their peripherals. For the serial interfaces, we provide the same `.swap()` and `.pins()` methods like megaTinyCore and MegaCoreX (which first introduced this feature) whereby each instance of a UART, SPI interface, or I2C interface can be moved appropriately, excepting SPI1 as noted above and described in detail in that library documentation.

Many libraries that are dedicated to specific peripherals also provide a method for choosing alternatepins, described in the library documentation
...
* TCB - TCBs are not for PWM. There is no support for changing their mapping. It is recommended to stick to the proper pwm timers, and use thise for everything else.
...
Like my other cores, Sketch -> Export compiled binary will generate an assembly listing in the sketch folder. A memory map is also created. The formatting of the memory map leaves something to be desired, and I've written a crude script to try to improve it.
...
**Long tones specifying duration** The three argument `tone()` counts the toggles of the pin for the duration. This means some very large numbers need to be handled. This core supports long tones through the three argument tone. A long tone is any tone where `4.294 billion < (frequency * duration) < 2.149 trillion` - by rearranging some divison, as long as 16k or more flash is present on the part we are compiling for (it uses more memory). Anything longer is a "very long" tone and is never supported via the three argument `tone()`. Your earsplitting high-pitched whine with a duration of more than a few hours (which is far outside the regime that tone is intended for) must be implemented with a 2-argument tone() and millis. If it needs to be implemented at all. Even normal "long tones" require a high frequency tone requested for minutes, which is an extreme use case.
...
### Serial constants have different numeric values
The serial constants are #defined as different numeric values from the official core. The official core was heinously inefficient, it had a lookup table for the options, to translate them back into values the chip understands, instead of simply defining them as that value inthe first place, saving memory, and improving execution time. Any code that uses the names like `SERIAL_7E2` or `SERIAL_8N1` will work the same way. If you instead figured out the numeric value that those constants have and passed that, it would not work correctly (who does that?!).
