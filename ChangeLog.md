...
* We weren't passing -mrelax to the linker. Without it, we got larger binaries because the linker missed opportunities to optimize jmp and call instructions (2-word instructions taking 3 cycles) to rjmp and rcall (1 word instructions taking 2 cycles). Ths magnitude of the improvement in binary size is surprising considering you're saving only 2 bytes and only on jumps and calls that didn't have this already, and it's clear that it does more than just replacing those instructions with the shorter ones (looking at assembly listings, the binary has been reorganized significantly). Anyway, it's not a risky optimization, and it should be turned on. I thought it was turened on, frankky.
...
* Fixed bug that caused analogWrite() to reject all pins as invalid (a simple typo - I'd meant to do an `if () {...}` structure spread across 3 lines inthe normal fashion - but instead wound up with an `if () {} ...` structure, so whether or not you passed and invalid pin, it will reject it and produce a compile error.
...
* Boardss.txt cleanup part 2, mnore progress
* Add menu to control printf implementation (there are 3 that make different flash vs feature tradeoffs.)
* Ensure that anything that determines what is built is always part of the name of expoered binaries. writing to SPM from app isn't, and this caused difficulty tracking down huge bug.
* Ensure that that functions in init do not get a ret autogenerated at the end. They must have attributes used _and_ naked. if t ((used)) they are optmimized away. If not ((naked)) they will get an autogenerated ret and the end which will generally result in a dirty reset loop. Also, if you put entry points in there that do have a return, thery need to start weith an rkmp to carry execution at startup around them.
* Documented reset flags and how important they are to check and clear. Optiboot does this and stashes them in reset in case you care (and resets if there are none, indicating that the board just jumped to 0 - likely due to a bug (a lot of memory corruption ends with this happening, which is why scrbbling ovver the ram with junk usually lands you at reset. Once a return address is damaged, is damaged, eventually it returns you to someplace that isn't whrewe the function was called from, likely empty flash skids along that until it wraps around and restarts from reset vector without reinitializing anything), and non-optiboot checks them in .init3 amd resets if there are none. But it doesn't reset them out of fear of breaking an application that needs to know them; so it's check is useless if you don't clear those flags.. Either be sure to clear them at start of setup, or (for production code) override init_reset_flags() and clear them there. See the new reset docs.
* Reorganized documentation dramatically. Less is in readme, more broken out into smaller files on individual topics.
* Add menu option to enable printf (and everything that uses it) to print floats.
* Fix bug where missing pin related macro on 28 pin parts related to wire1 would cause compile errors (may not be in any release)
* Fix bug whrre under some conditions, errors would be encoutnered for 32-pin parts due to pinmappings missing for USARRT2 (not in any release)
* Add (commented out) entries to boards.txt for DD series (only one thing is not known: size of bootsize/codesize segments) and EA without optiboot.
* reimplemented attachInterrupts to have a separate file for eaach port.  I doesn't do what I'd hoped to be able to do, get it to automatically only include ports you attach to; so I default to it being on all ports but have menu option to either do that manually - or revert to the old version if I broke something. Note: I didn't bother fixing PORTG on the old version implementation. That never worked, and nobody noticed.
* I finally checked on hardware, MVIO being disabled just seems to turn off the oversight of that behavior. With it disabled it does not short VDDIO2 to power rails as some, including myself, had feared it might. MVIO menu is back. I suspect the reason to disable it is power usage, because it's basically another thing to monitor like BOD.
...
* Add teaser for possible future improvements to export compiled binary hex listings and memory map (for hex listings, I just want to get rid of the irrelevant and stupid offsets relative to randomly chosen constants, instead of usefui ones, for the map, it needs the columns fixed, or at least the formatting tweaked so you could import it into a spreadsheet. Right now it requires manual regex step just to get it clean enough that excel can import it... We now pull in a python installation for SerialUPDI, why not get some more use out of it processing the crap avrgcc gives us to work with.)
