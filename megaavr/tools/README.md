...
##### Note - those numbers were taken with SerialUPDI 1.1.0, versions of the core with later versions (megaTinyCore 2.4.0 and DxCore 1.4.0) make use of read chunking (which has a small effect on verify speed) except on the TURBO 345600 baud modes to improve compatibility with serial adaopters. On tinyAVR those also add a small delay after writing each page; this isn't necessary with all adapters (often the USB latency is enough - I have never had a problem with it on CH340/Windows.
