...
void setup() {
  ...
  byte flashcheck = Flash.checkWritable();
...
void loop() {
  ...
  Serial.println(pgm_read_word_far(0x180FE), HEX);
  if (pgm_read_word_far(BASE_ADDRESS + 0xFE) == 0x0001) { //For this demo, first page after base
  ...
  Serial.print(Flash.erasePage(BASE_ADDRESS));
  ...
  Serial.printHex(Flash.writeWord(BASE_ADDRESS + 1, 0x1234));
  Serial.println();
  Serial.println(F("Byte to base + 1 allowed by Flash.h ->"));
  Serial.printHex(Flash.writeByte(BASE_ADDRESS + 1, 0x01));
  Serial.println();
  /*  Why did I decide to only allow bytes to be written unaligned? Well, in order to dodge the errata on some of the 128k parts
   *  we can only use SPM to write. SPM writes a word, not a byte. The only way to write a byte is to set one half of a word to
   ...
   *  unchanged. With larger datatypes, just goddamn align them! But if you're writing independent bytes, well, am I to tell you
   *  not to use half the space? That seems s bit too forceful...
   */


  Serial.println(F("Word to base + 2 status ->"));
  Serial.printHex(Flash.writeWord(BASE_ADDRESS + 2, 0x5634));
  Serial.print(' ');
  Serial.printHex(Flash.writeWords(BASE_ADDRESS + 0x06, WordArray, 5));
  Serial.print(' ');
  ...
  Serial.printHex(Flash.writeWords(BASE_ADDRESS + 0x10, wordptr, 10));
  Serial.println();
  delay(500);
  readBackDemo();
  ...
  Flash.writeWord(BASE_ADDRESS + 0xFE, 0x0001);
  readHalfPage(BASE_ADDRESS);
  delay(60000);
}
...
void readHalfPage(uint32_t StartAddress) {
  ...
  for (unsigned long i = StartAddress; i < (StartAddress + 0x100); i++) {
    ...
    Serial.printHex(pgm_read_byte_far(i));
    if ((i & 31) != 31) {
...
void readBackDemo() {
  ...
  for (byte i = 0; i < 16; i++) {
    ...
    Serial.print(pgm_read_byte_far(BASE_ADDRESS + i), HEX);
  }
  Serial.println();
  delay(500);
  ...
  for (byte i = 0; i < 16; i += 2) {
    if (i) Serial.print(':');
    Serial.print(pgm_read_word_far(BASE_ADDRESS + i), HEX);
  }
  Serial.println();
  Serial.println();
  Serial.println();
  ...
  for (byte i = 0; i < 20; i++) {
    if (i) Serial.print(':');
    Serial.print(pgm_read_byte_far(BASE_ADDRESS + 0x10 + i));
  }
  Serial.println();
  ...
  for (byte i = 0; i < 10; i++) { //here instead of incrementing i by 2
    if (i) Serial.print(':');
    Serial.print(pgm_read_word_far(BASE_ADDRESS + 0x10 + i * 2)); //multiply i by 2 down here
  }
  Serial.println();
  Serial.println(F("Whaaa? Okay how bout we try again as hex!"));
  for (byte i = 0; i < 20; i++) {
    if (i) Serial.print(':');
    Serial.print(pgm_read_byte_far(BASE_ADDRESS + 0x10 + i),HEX);
  }
  Serial.println();
  delay(500);
  for (byte i = 0; i < 10; i++) { //here instead of incrementing i by 2
    if (i) Serial.print(':');
    Serial.print(pgm_read_word_far(BASE_ADDRESS + 0x10 + i * 2),HEX); //multiply i by 2 down here
  }
  Serial.println();
