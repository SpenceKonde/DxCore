...
uint8_t flashErasePage(const uint32_t address, const uint8_t size) {
  if (FUSE.BOOTSIZE != 0x01) {
    return FLASHWRITE_NOBOOT;
  }
  uint8_t command;
...
uint8_t flashWriteByte(const uint32_t address, const uint8_t data) {
  ...
  #else
    // if we were to decide we're okay with unaligned writes
    // it we could always just put 0xFF in the high byte and
    dataWord=0xFF00 | data;
    uint16_t zaddress = address; //just truncate it
  #endif
  do_nvmctrl(NVMCTRL_CMD_FLWR_gc);
  __asm__ __volatile__(
            "mov  r0,%A[dat]"                     "\n\t"
...
uint8_t flashWriteWords(const uint32_t address, const uint16_t* data, const uint16_t length) {
  if (length == 0) {
  ...
  // This would just be a damned mess to handle!
  #if (!defined(ALLOW_UNALIGNED))
    if (address & 0x01) {
      return FLASHWRITE_ALIGN;
    }
