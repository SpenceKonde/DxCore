...
| TIMER       | On DA/DB | On DD | On EA | Pins:                                                  |
|-------------|----------|-------|-------|--------------------------------------------------------|
| TCA0        | Yes      | Yes   | No    | Pins 0-5 on any single port. Never split between ports |
| TCA1        | >32 pin  | No    | Yes   | Pins 0-5 on PB or PG, else pins 4-6 of PC, PE, PA* PD* |
| TCD0        | Yes      | Yes   | No    | PA4-7 on DA/DB. DD has special split-port mux option   |
| TCB0        | Yes      | Yes   | Yes   | PA2 or PF4. All TCBs are poor PWM timers               |
| TCB1        | Yes      | Yes   | Yes   | PA3 or PF5.                                            |
| TCB2        | Yes      | 28/32 | Yes   | PC0 or PB4. Default millis timer `**`                  |
| TCB3        | 48/64 pin| No    | Yes   | PB5 or PC1.                                            |
| TCB4        | 64 pin   | No    | No    | PG3 or PC6.                                            |

`*` MUX options for PA/PD on TCA1 are EA-series only. On DA-series, only PB and PC mux options work.
`**` A TCB cannot be used for PWM if being used for millis. On DD-series with 14 or 20 pins, TCB1 is the default millis timer. Otherwise, TCB2 is. TCB2 is always the default millis timer if it exists. A TCA can be used for millis, though it is less accurate, and it can output PWM and act as the millis timer. TCD as millis is not supported because TCD is *really* not meant for that kind of thing. and is an outstanding PWM timer (see [TCD reference for specific "can I change this without breaking stuff" questions re: TCD0](Ref_TCD.md). We are more forgiving of users tweaking the settings of TCD and wantign to use analogWrite with it because TCD0 is very confusing to configure, and we believe that the number of people who want to be able to tweak it's PWM is markedly larger than the number of people who can figure out how to manually configure it after taking over TCD0.
