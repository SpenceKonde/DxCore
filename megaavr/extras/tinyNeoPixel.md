...
### Library name
While this library was initially created in order to both ensure compatibility with, and through the Static version, fit within the flash and memory constraints of the tinyAVR line of parts, **this library is entirely suitable for non-tiny devices**, and we belive it to be strictly superior to Adafruit's library; It offers all the functionality of the Adafruit version on the library, with the addition of the Static mode. Where appropriate, core-specific matters are accounted for (not as relevant for modern AVRs).
...
```c++
    ...
    NEO_RBG /* Everything else is essentially unheardof */
    NEO_GBR
    NEO_BRG
    NEO_BGR
```
...
## Refresh Rate
Data is sent at 800 kbit/s, or 100k byte/s - 33,333 LEDs/s. Latching time - in theory - takes 50us of idle time (40 LEDs of data). On original WS2812's, the actual latching time was just about 6us (5 LEDs of data), while versions have since been produced that wait as long as 200-250us to latch! Note: Although show() guarantees that it can't be called for 50 us after it last returned. This check relies on micros() and is disabled when millis timekeeping is not used or is based on the RTC (hence no micros() is available) as well as on chips with less than 8k of flash (since micros() is painfully large to pull in). A pragma message is issued in these cases. At these times, it's up to you to ensure that there's a brief pause between calls to show() if you aren't doing anything that takes long enough, like updating the colors of the pixels you're going to send (if you aren't doing that, why are you sending data again?). In any event, you will likely want some mechanism to issue updates to the LEDs at a vaguely constant rate.
...
* The 5050 package with 6 pins was used by the original WS2812, and is now used by the WS2813 and WS2815, and a small number of compatible clones. It is also used by a very large number of incompatible leds. If you see leds with a 6-pin package and were considering buying them, you need to do a bit of further investigation. There are three possibilities, and only one of them is appropriate if you wanted 2812-alikes. The pinout chart will hold the answers; if it's strip, you can probably get the key information from the markings on the strip:
  * If they have two Vcc pins per LED, or (for strip) if there is only one data line, they are actual WS2812-not-B's. You don't want these. They burned out easily and had no particular advantage. These are quite rare now, having been replaced with other types of LEDs
