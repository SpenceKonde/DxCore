...
Like the normal Adafruit_NeoPixel library, this supports WS2811/WS2812/SK6812 and all the nominally compatible single-wire individually addressable LEDs. It does not support two-wire ones like the APA102 or the countless knockoffs thereof; libraries for those do not pose special compatibility issues with AVRxt because they are not timing critical in the same way. Also like the original, this library takes the datasheet at it's word. In fact, many of the timing numbers in the datasheet are wrong (time to latch pixels is only a few us, not 50us for the ones from WorldSemi - but some with MUCH longer delays are available, often not marketed differently...)

At the extremes of clock speed, this implementation gets kind of absurd, spending the vast majority of the sending time in various cycle-counting delays. The next hope, of course, is to find a way to run it in the background; I think this is actually possible on an overclocked Dx-series, but I haven't found time to implement it yet - it would be a new library (if it's readily libraryable. )
