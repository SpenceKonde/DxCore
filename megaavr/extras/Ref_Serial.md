...
* Runing a single combined TX and RX line is not uncommon, particularly when RS-485 is in use.
...
The synchronous mode isn't as flexible as you might think, largely because of the whole start/stop bit thing. Another reason is is less frequently used is the availability of adapter harware for desktop compters, or rather the lack thereof. It is, if anything, even harder to find support for than a SPI.

Note that, unlike the MSPI mode, Syncronous Mode does support opperation as either slave or master; the MSPI mode, which drops the start and stop bits, and works largely like a proper SPI interface is master only.
...
It would come as no surprise to the owner of that hypothetical quarrelsome quartet when their new tinyAVR couldn't talk to some of those. However, someone who had been using classic AVRs with crystal in an existing setupt to talk to eachother might be unaware this, and blame the modern AVR part (I have had that discussion a few times)
