...
Not much of a choice was left for the core designer - the pins that the high council blessed at TCB capable, we only have one per timer. Note TCB2 by default is used so PC0 PWM should not be expected with millis one.
