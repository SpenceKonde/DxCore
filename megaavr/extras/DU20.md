...
### Fully async pins
All pins are fully async, able to wake from sleep without clock on Rising and Falling edges not just change and low level, able to respond to events shorter than 1 system clock, amd demomstrate accordigly less noise immunity :-P. On earlier parts only pins 2 and 6 in each port were
...
|---------|------|-------------------|-----------------|
| ALT3    | 3    | SDA/PA0 SCL/PA1   | None            |



### TCA mux options
You really don't use a DU for PWM. It's still credibly for the purpose, just limited in the number of channels. Since, by fefault the core uses split mode to maximize pwm channels while replicating the original PWM behavior of 8-bit analogWrite()
