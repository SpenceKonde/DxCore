...
* TCA1 is by default set to PORTB, like on the DA's. Again, this decision is dubious on the DBs where the PORTMUX bug is not present, but since the port assignments are easy to change, I think aligning with the DA64 still made sense. If I could do it all over again, I'd probably default PG for TCA1, and PF for TCA0.
...
PORTMUX.TCAROUTEA = (PORTMUX.TCAROUTEA & PORTMUX_TCA1_gm) | PORTMUX_TCA0_PORTD_gc; // Move TCA0 PWM to PORTD but don't change TCA1
