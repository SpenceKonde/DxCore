...
#### TCE0 Mux Options
Our primary timer for PWM is the TCE. We have the following mux options for it.

| Port    | Number | *WO0*  | *WO1*  | *WO2*  | *WO3*  | WO4   | WO5   | WO6   | WO7   |
|---------|--------|--------|--------|--------|--------|-------|-------|-------|-------|
| PORTA   |   0    | PA0    | PA1    | PA2    | PA3    | ~PA4~ | ~PA5~ | ~PA6~ | ~PA7~ |
| PORTC   |   2    | PC0    | PC1    | PC2    | PC3    | -     | -     | -     | -     |
| PORTD   |   3    | PD0    | PD1    | PD2    | PD3    | ~PD4~ | ~PD5~ | ~PD6~ | ~PD7~ |
| PORTF   |   5    | PF0    | PF1    | PF2    | PF3    | ~PF4~ | ~PF5~ | -     | -     |
| PORTC2  |   8    | PA0    | PA1    | PC0    | PC1    | ~PC2~ | ~PC3~ | -     | -     |
| PORTA2  |   9    | PA2    | PA3    | PA4    | PA5    | ~PA6~ | ~PA7~ | -     | -     |


This timer gives us only FOUR outputs, even though it has 8 pins - we can get inverted outputs out of the other half of the pins with independent dead time generation if we happen to vbe driving a brushless DC motor or multiphase DC-DC converter. We're probably not.

It is, as yet, unclear whether WEX Luther can be convinced to move outputs to the other pins; the content of the relevant sections (of the 2 longest chapters on an AVR peripheral in the modern AVRs - WEX and TCE are the top two, followed by TCD and TCA *I'm sure he can! Don't take that from a peripheral!*



As the largest chip of he set, I've attempted to extrapolate what the master chart in the AVR war room must have listed for the TCE portmux; I don't think I'm going out on a limb here.

| Port    | Number | *WO0*  | *WO1*  | *WO2*  | *WO3*  | WO4    | WO5    | WO6   | WO7   |
|---------|--------|--------|--------|--------|--------|--------|--------|-------|-------|
| PORTA   |   0    | PA0    | PA1    | PA2    | PA3    | PA4    | PA5    | PA6   | PA7   |
| PORTB   |   1    | PB0    | PB1    | PB2    | PB3    | PB4    | PB5    | PB6   | PB7   |  Theorized, should a >32 pin part with a TCE come out. The EC is the obvious candidate, as it's known to be planned for a 48-pin version.
| PORTC   |   2    | PC0    | PC1    | PC2    | PC3    | PC4    | PC5    | PC6   | PC7   |  The higher PC's are so obvious that I think they can be taken for granted.
| PORTD   |   3    | PD0    | PD1    | PD2    | PD3    | PD4    | PD5    | PD6   | PD7   |
| PORTE   |   4    | PE0    | PE1    | PE2    | PE3    | PE4    | PE5    | PE6   | PE7   |  Theorized
| PORTF   |   5    | PF0    | PF1    | PF2    | PF3    | PF4    | PF5    | -     | -     |  PF6 is reset and has no output drivers, PF7 is UPDI, precedent is timers don't get UPDI or reset pins that one might otherwise expect from the mapping pattern
| PORTG   |   6    | PG0    | PG1    | PG2    | PG3    | PG4    | PG5    | PG6   | PG7   |  Theorized
...
TCE is portmux aware, and we set the portmux to PORTC by default, since that will give a full suite of PWM pins to all parts out of the box.

Note that we now have a 4-bit mux option register, and there are two whacko pinouts.



##### TCE + WeX advanced features
Infamous supervillian WeX Luther is back! (*Superman's enemy? That's "Lex Luther"* "This one definitely said his name was WeX..." *You've got the wrong guy, bro* "Oh yeah? Read this chapter of the datasheet about it. I think you'll have to agree that villianry was involved...." *Wow, long chapt-* "- and you'll need the TCE chapter, can't understand one without the other" *Ugh, what are these, the two longest chapters?* "Yes. Oh, and here, CLKCTRL - that's important here too." *Jeez* "The TCA was already one of the longest chapters in the datasheet (largely because of the split/single mode thing, which we lost)" *Boooooo!* "and TCE filled that space and more, then WeX himself went on for as long again (much of it somewhat incoherrent, but it was gonna change the world)." *supervillians changing the world... couldn't go wrong* "I think it was 'change' - something like that" *What are all these cool features?* "Uhm, there's HiRes, and uh, well if you were driving brushless DC motors or synchronous DC-DC converters, there's programmable deadtime insertion. And some of the TCD-like fault detection stuff in WeX, which is also for BLDC and switch mode applications"
...
Now, knowing our powers of (2<sup>8</sup>), we know 2^24 is 16.7 million. That means clocked with no prescaler from the CPU, it would take just over a second, or about 800ms to overflow. The PLL only lowers that by a factor of 4. Basic math tells you that those extra bits would be of dubious utility if you'd been thinking about 24-bit PWM (which is more precision than you need by far). And sure enough, that's not what we get either. Instead, much like the TCBs,one of the available modes is 8-bit PWM, working very much the same as TCB's PWM mode, other than that there are two channels.

The TCF's featureset seems barebones compared the the monstrosity that is TCE, See [[Ref_Timers.md]]
...
~TCF for millis is under consideration~ See errata, that put the kibosh on that idea right quick.  It would be kinda cool to have a micros interrupt that only fired every sixteen seconds, but not with the threat of unexpected timetravel.
...
**Datasheets and eratta change. You can sign up to get emails about such changes through the Microchip PCN system; if you don't, be sure to always use the latest version of the datasheet and especially the errata**
