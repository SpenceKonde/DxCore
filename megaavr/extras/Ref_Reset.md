...
When you see unexpected resets, check the reset cause! If it's showing as software reset - this is likely what's happening. You can override init_reset_flags() to turn on LEDs or something like that; you are forced to work under very painful conditions: There's no serial, timekeeping is out of whack, and so on - the system is not so hosed that you can't turn on a pin, and you can generate delays with delay_us() and delay_ms() in <util/delay.h) (provided they are compile time known constant delays) and use that to blink a LED, thought the rate at which it is blinking will be wrong.). The best way to avoid this problem is to preempt it. Test often. If suddenlty it starts throwing dirty resets, well, you know what you were working on before this started happening. Did you add some code to write to an array? Maybe you wrote past the end of it. Did you add an interrupt? Are you sure the vector is spelled right and you're enabling the right interrupt? (see below for more causes of a dirty reset). Dirty resets are BY FAR the most common cause of hangs and Arduinos getting into a "bad state"; the recent changes should make these far less likely to result in anything more than a surprise reset. Which is bad, but not as bad as the part hanging until you reset it manually.
