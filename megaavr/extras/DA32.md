...
### PWM Pins
* TCA0 is by default set to PORTD. This is a decision I would have made differently had I owned a crystal ball during the design process (frankly, it's not the easiest decision to defend even assuming I had known what was coming); but by the time the DB's had arrived with MVIO eating a PORTD pin AND all the OPAMPS on PORTD, the core had been released and we don't like to break backwards compatibility. It's not much of an issue now that you can trivially switch the ports at runtime. Were I doing this today, I'd default PORTF
