...
#define portModeRegister(P)   ((volatile uint8_t *)(&portToPortStruct(P)->DIR))
  ...
  //Number being 0 or 1 or 255 to pick the lowest numbered one not set. Returns event channel number TBD if that should be the EVSYS values or 0 or 1. If "Pick unused one" is requested but both already assigned, will return 255
