...
/*      ID_AVR_SD       (0x80) */ // Should be the DS. It's similar to DA, DB, DD, but with Security.
/*      ID_AVR_??       (0x88) */ // EH/DH? For "Huge" the
#define ID_AVR_EB       (0xC0)
...
// _AVR_FLASH gives the flash size as a num
  ...
  #define __AVR_DU__
#elif defined(__AVR_AVR16EA48__) || defined(__AVR_AVR64EA48__) || defined(__AVR_AVR32EA48__)
  #define EA_48_PINS
  #define HAS_48_PINS
  #define EX_48_PINS
  #define CORE_PART_ID_LOW (ID_48_PINS | ID_AVR_EA)
  #define __AVR_EA__
#elif  defined(__AVR_AVR16EA32__) || defined(__AVR_AVR64EA32__) || defined(__AVR_AVR32EA32__)
  #define EA_32_PINS
  #define HAS_32_PINS
  #define EX_32_PINS
  #define CORE_PART_ID_LOW (ID_32_PINS | ID_AVR_EA)
  #define __AVR_EA__
#elif  defined(__AVR_AVR16EA28__) || defined(__AVR_AVR64EA28__) || defined(__AVR_AVR32EA28__)
  #define EA_28_PINS
  #define HAS_28_PINS
  #define EX_28_PINS
  #define CORE_PART_ID_LOW (ID_28_PINS | ID_AVR_EA)
  #define __AVR_EA__
#elif  defined(__AVR_AVR16EB20__) || defined(__AVR_AVR32EB20__)
  #define EB_20_PINS
  #define HAS_20_PINS
  #define EX_20_PINS
  #define CORE_PART_ID_LOW (ID_20_PINS | ID_AVR_EB)
  #define __AVR_EB__
#elif  defined(__AVR_AVR16EB28__) || defined(__AVR_AVR32EB28__)
  #define EB_28_PINS
  #define HAS_28_PINS
  #define EX_28_PINS
  #define CORE_PART_ID_LOW (ID_28_PINS | ID_AVR_EB)
  #define __AVR_EB__
#elif  defined(__AVR_AVR16EB32__) || defined(__AVR_AVR32EB32__)
  #define EB_32_PINS
  #define HAS_32_PINS
  #define EX_32_PINS
  #define CORE_PART_ID_LOW (ID_32_PINS | ID_AVR_EB)
  #define __AVR_EB__
#elif  defined(__AVR_AVR16EB14__) || defined(__AVR_AVR32EB14__)
  #define EB_14_PINS
  #define HAS_14_PINS
  #define EX_14_PINS
  #define CORE_PART_ID_LOW (ID_14_PINS | ID_AVR_EB)
  #define __AVR_EB__
#else
  #error "Can't-happen: unknown chip somehow being used - and we detect every Dx and Ex chip announced!"
#endif
/*  _AVR_CLOCKMODE
 * 0b rrrr rrPP TTTA UESW
 * r   = reserved
 * PP  = PLL capabilities:
 *    00 = no user-facing PLL, like the DU or tinyAVR  (the DU has one internally that it uses for USB but you could determine that only if you happened to know that USB requires a 48 MHz
 *         clock and the only way it is gonna get that from a a slower clock is with a PLL. One has to assume that the DU's PLL was the one the earlier Dx's used for their clock
 *    01 = DA/DB=style PLL to clock a TCD. Mul 2/3x (4x undoc, works). Max 48 MHz per spec. I have seen the TCD working without glitches at 128 MHz (it got mad glitchy at 160 MHz though, guess that was asking too much). Can't clock anything other than the TCD
 *    10 = EB-style ballerass PLL - input clock is prescaled by 1, 2, 4, or 6; PLL multiplies clock by 8 or 16x, max rating 80 MHz. Then the PLL itself can divide the output by two; finally the prescaler now has TWO outputs, CLK_PER and CLK_PER4.
 * TTT = Tuning field size: 0 = 32 (DX), 1 = 64 (t0/1), 2 = 128 (t2)
 * A   = 1 = Supports autotune
 * U   = 1 = Supports autotuning from USB (USB devices only)
 * E   = 1 = Supports ext HF crystal
...
#define _AVR_EX_SERIES          (0x20); //
  ...
  #define     _AVR_CLOCKMODE    (0x0113) // Crap manual tuning, autotune, no crystal, clock select, supports RTC xtal.
  #define     _AVR_FLASHMODE    (2)
#elif defined(__AVR_DB__)
  #define     _AVR_GENUS        _AVR_DX_SERIES
  #define     _AVR_FAMILY       ("DB")
  #define     _AVR_CLOCKMODE    (0x0117) // Crap manual tuning, autotune, crystal, clock select, supports RTC xtal.
  #define     _AVR_FLASHMODE    (2)
  ...
  #define     _AVR_CLOCKMODE    (0x0115) // Crap manual tuning, autotune, crystal, base clock in fuses like a tiny, supports RTC xtal.
  #define     _AVR_FLASHMODE    (3)
#elif defined(__AVR_EB__)
  #define     _AVR_GENUS        _AVR_EX_SERIES
  #define     _AVR_FAMILY       ("EB")
  #define     _AVR_CLOCKMODE    (0x0215) // Okay, the EA's clock system was boring AF. That is not the case here. That PLL comes with a whole crate of new functionality, and is integral to them new timers and their fancy functionality.
  #define     _AVR_FLASHMODE    (3)
#else
  ...
  #define                     _AVR_FLASH   (256) //Not yet observed
#elif PROGMEM_SIZE == 0x20000
  #define                     _AVR_FLASH   (128)
#elif PROGMEM_SIZE == 0x10000
  #define                     _AVR_FLASH    (64)
#elif PROGMEM_SIZE == 0xC000
  #define                     _AVR_FLASH    (48)
#elif PROGMEM_SIZE == 0x8000
  #define                     _AVR_FLASH    (32)
#elif PROGMEM_SIZE == 0x4000
  #define                     _AVR_FLASH    (16)
#elif PROGMEM_SIZE == 0x2000
  #define                     _AVR_FLASH     (8) /* May not be seen on Dx/Ex! Both 8's have been canceled! It does mean they don't have to manage the different behavior of 8k AVRs (no jmp, small vectors - multiple serious problems in releases.*/
#elif PROGMEM_SIZE == 0x1000
  #define                     _AVR_FLASH     (4) /* Unlikely to be seen on Dx */
#elif PROGMEM_SIZE == 0x0800
  #define                     _AVR_FLASH     (2) /* Unlikely to be seen on Dx */
#endif

/* Why not #define _AVR_FLASH      (PROGMEM_SIZE/1024)   wouldn't this work for all future parts?*/
...
/* Okay done with standard features */
  ...
  #elif (PROGMEM_SIZE == 0x4000)  // 16k EA/EB
...
/* CLKCTRL *
 * Not needed, everything is either paired with hardware (and optional) and widely distributed or is unique and very different.
 *
 */
 ...
 * Too bad they released so many parts with the other versions :-/ */
  ...
  // Ex-series, with EVGENCTRL registers on RTC and PORT.
  ...
  #define _AVR_OPAMP_COUNT   (0)
    ...
    #define _AVR_CCL_CLOCK_OPTS (0x02)
    /* 0x01 = IN2, CLK_PER, OSC_HF, 32K_INT and 1K_INT, and the PLL. I always wondered why the Dx couldn't do that - it's PLL is only supposed to be able to do 48 MHz, and the CCL inverting it's own output would oscillate at over 80 MHz */
  #else
    /* 0x01 = IN2, CLK_PER, OSC_HF, 32K_INT and 1K_INT.  */
    #define _AVR_CCL_CLOCK_OPTS (0x01)
    /* 0x00 = IN2 or CLK_PER only. */
    ...
    #define _AVR_LUT_COUNT     (12) // and 4 LUTs are more than twice as useful as two, and so on. I'm not sure it's quantifiable but...
...
#endif
// Will have two waveform outputs in an 8-bit PWM mode; there are also several other modes which make little sense to the lay person. Actually, I think I'm going to need to try them to figure out what they do.
...
//The new crystal clock support brought a bug along though.
  ...
  #define ERRATA_PLL_XTAL                (0x10) /* PLL cannot use external xtal clock source.  Fixed in Rev B0                                             */
  #define ERRATA_2V1_EXCESS_IDD          (0x10) /* When VDD falls to the range 1.9-2.1V (typ) from above that range, 1.5mA additional current drawn Fixed in Rev B0!  */
#else
  #define ERRATA_PLL_XTAL                (0) /* DA has no crystal and DU has removed the PLL from user control to provide USB with the requisite 48 MHz clock, and the EB is all different */
  #define ERRATA_2V1_EXCESS_IDD          (0) // DB Only
#endif
...
// And both DA and DB had a whole slew of issues
  ...
  #define ERRATA_TCA_RESTART             (1) // It resets the direction, like the datasheet said. Both of these arte incorrect, it is not supposed to do that.
  ...
  #define ERRATA_USART_ISFIF             (1) // Seems universal. Not a big deal. We put the code in to catch it so it's no concern.
  #define ERRATA_USART_WAKE              (1) // Again, nearly universal
#elif defined(__AVR_DB__)
  #define ERRATA_TCD_PORTMUX             (0x10) // One of the most irritating erratums
  #define ERRATA_DAC_DRIFT               (0x10) // How much drift? I dunno - enough for Microchip to feel a need to add an erratum about it.
  #define ERRATA_TCA_RESTART             (0x10) // Prior to B0, it did exactly what the datasheet said - restart event's reset direction.
  #define ERRATA_CCL_PROTECTION          (0x10) // Busted on all pre-DD parts
  #define ERRATA_TCD_ASYNC_COUNTPSC      (0x10) // Busted on all pre-DD parts
  #define ERRATA_TCB_CCMP                (0x10) // Busted on all pre-DD parts
  #define ERRATA_TWI_PINS                (0x10) // Busted on all pre-DD parts
  #define ERRATA_USART_ONEWIRE_PINS      (0x10) // Busted on all pre-DD parts.
  #define ERRATA_TWI_FLUSH               (1)    // NOT fixed!
  ...
  // demonstrating bugs
  #define ERRATA_TCD_HALTANDRESTART      (1) /* This is the very complicated problem with halt and sw restart mode in certain configurations */
#elif defined(__AVR_EA__)
  #define ERRATA_NO_MULTIPAGE_UPDI       (1) // "Good news, Flash Multipage Erase no longer bypasses write protect!" "But now it doesn't work at all" "Only over UPDI" *So it will only help your bootloader* "Once I get my hands on some rev B2 hardware"
  #define ERRATA_FULL_CRC_ONLY           (0x12) // who cares? I mean, I guess anyone who wants to issue firmware updates and use the CRC feature. Which like, would be a good idea if they're doing f/w updates in the field. Not the reason for the die rev though.
  #define ERRATA_USART_ISFIF             (1)    // ISFIF is enough of a non-issue with straightforward handling that I doubt this is a priority, as autobaud is not exactly widely used, especially as internal oscillators are now easily good enough for UART.
  #define ERRATA_NO_ERASEWRITE           (0x12) //
  #define ERRATA_FLASH_ENDURANCE_BOD3    (0x12) // "Just to add some insult to your injury (or the other way around, frankly I can't tell which of the NVM issues is more injurious and which is more insulting)"
  #define ERRATA_2V7_MIN_FLASH           (0x12) // *Depends on whose perspective. The guys running at 2.5V are quite troubled by the no-writes below 2V7; your bootloader is going to only work on some EA's due to the erasewrite issue,
  //                                               and the chips themselves said forming memories hurt, so I think the write ensurance would have their vote for most injurious.* ""

/* *Hmm, as I'm reading that, there's no way to do self programming in C and be guaranteed safe output, because there's no way to prevent C from executing ld, ldd, or lds on the NRWW section. But a tight bootloader might not need to ever do something like
    that; they say instruction fetch is okay, and why would you be reading data from the bootloader section of the flash? I don't think any of the actual software that implements subsets of STK500 would do any of those things - like, the protocol allows it
    but I don't think any contemporary implementations of stk500 and optiboot with a naive port would run into trouble here*
   "Oh, and that bit about multipage flash erase not causing problems for bootloader? You know we never used multipage erase right? I tried, but the overhead of switching commands ends up taking way too much flash."

*/
#elif defined(__AVR_DU__)
  #define ERRATA_USB_ODD_POINTER         (1)
  #define ERRATA_USART_ISFIF             (1) // Mmm - no, I guess it wasn't a matter of catching it in time. I just don't think this is considered enough of a problem. I don't disagree. It's easy to work around, and involves handling error conditions
...
// I/O registers do it, and I suspect that RSTCTRL's registers would as well, except both of them are special: RSTFR one of the wackiest registers around, and SWRR is a protected timed write sequence which can't manifest the bug. SLPCTRL's secomd register
// is also protected, if it exists (also, it just screams "development functionality")
//  (note: I predict that you can't trigger the bug with the flag registers either, ie, VPORTx.INTFLAGS

/***********************************************************************************************************************************************\
 * CORE AND HARDWARE FEATURE SUPPORT (ie, these are meant to be checkable - though usually if you care, you can't do anything graceful           *
...
#if (defined(TCB_CLKSEL2_bm) || defined(TCB_CLKSEL_2_bm)) // 1.1 names
 ...
 */
  ...
  #if defined(TCB4)
    #if defined(EVSYS_CHANNEL0)
      #define EVSYS_CHANNEL0_TCB4_CAPT_gc EVSYS_CHANNEL_TCB4_CAPT_gc
      #define EVSYS_CHANNEL0_TCB4_OVF_gc EVSYS_CHANNEL_TCB4_OVF_gc
    #endif
    #if defined(EVSYS_CHANNEL1)
      #define EVSYS_CHANNEL1_TCB4_CAPT_gc EVSYS_CHANNEL_TCB4_CAPT_gc
      #define EVSYS_CHANNEL1_TCB4_OVF_gc EVSYS_CHANNEL_TCB4_OVF_gc
    #endif
    #if defined(EVSYS_CHANNEL2)
      #define EVSYS_CHANNEL2_TCB4_CAPT_gc EVSYS_CHANNEL_TCB4_CAPT_gc
      #define EVSYS_CHANNEL2_TCB4_OVF_gc EVSYS_CHANNEL_TCB4_OVF_gc
    #endif
    #if defined(EVSYS_CHANNEL3)
      #define EVSYS_CHANNEL3_TCB4_CAPT_gc EVSYS_CHANNEL_TCB4_CAPT_gc
      #define EVSYS_CHANNEL3_TCB4_OVF_gc EVSYS_CHANNEL_TCB4_OVF_gc
    #endif
    #if defined(EVSYS_CHANNEL4)
      #define EVSYS_CHANNEL4_TCB4_CAPT_gc EVSYS_CHANNEL_TCB4_CAPT_gc
      #define EVSYS_CHANNEL4_TCB4_OVF_gc EVSYS_CHANNEL_TCB4_OVF_gc
    #endif
    #if defined(EVSYS_CHANNEL5)
      #define EVSYS_CHANNEL5_TCB4_CAPT_gc EVSYS_CHANNEL_TCB4_CAPT_gc
      #define EVSYS_CHANNEL5_TCB4_OVF_gc EVSYS_CHANNEL_TCB4_OVF_gc
    #endif
    #if defined(EVSYS_CHANNEL6)
      #define EVSYS_CHANNEL6_TCB4_CAPT_gc EVSYS_CHANNEL_TCB4_CAPT_gc
      #define EVSYS_CHANNEL6_TCB4_OVF_gc EVSYS_CHANNEL_TCB4_OVF_gc
    #endif
    #if defined(EVSYS_CHANNEL7)
      #define EVSYS_CHANNEL7_TCB4_CAPT_gc EVSYS_CHANNEL_TCB4_CAPT_gc
      #define EVSYS_CHANNEL7_TCB4_OVF_gc EVSYS_CHANNEL_TCB4_OVF_gc
    #endif
    #if defined(EVSYS_CHANNEL8)
      #define EVSYS_CHANNEL8_TCB4_CAPT_gc EVSYS_CHANNEL_TCB4_CAPT_gc
      #define EVSYS_CHANNEL8_TCB4_OVF_gc EVSYS_CHANNEL_TCB4_OVF_gc
    #endif
    #if defined(EVSYS_CHANNEL9)
      #define EVSYS_CHANNEL9_TCB4_CAPT_gc EVSYS_CHANNEL_TCB4_CAPT_gc
      #define EVSYS_CHANNEL9_TCB4_OVF_gc EVSYS_CHANNEL_TCB4_OVF_gc
    #endif
  #endif
...
has once worked for the same thing as meaning that thing */
   ...
   * xmega line   *
   * But regardless, the xmegas are not a rolemodel of success, even if they have their high points. They clearly know this at Microchip
   * xMega appears to have been buried in a shallow grave shortly after Microchip showed up
   *
   ...
   *
   * If chip designs are cars, I'm imagining these designers in a lot full of old weatherbeaten devices, scrounging for peripherals to soup up is nes
   * "AVRxm" chip they were working on.
   * "Hey how bout this CCL doohickey?"
   * "Too complicated, boss said simplifying the product line! Look at all this crap, what the hell were we thinking combining the CCL and timers?!"
   * "Lets ditch the timer and all those wacky accessories. Just 2 luts and a sequencer"
   * "What good is that? You can't do much with 2x 2 input LUTs..."
   * "No, but look, without all this junk, we could just run a third input to the LUT!"
   * "Oh, great, throw it in and come over to review the timers we've found and pick the ones
   * we want to use."
   * "K - ah, where are we meeting?"
   * "See those two large contraptions over there? Between those. The boss is pushing for the one
   * on the left"
   * "That monstrosity?! What the hell is it even?"
   * "It was a t861's T1, then it was a PSC... now he calls it TCD...
   * "I thought you said he wanted to keep it simple?
   * "Wait till you see the TCE."
   * "What good is that without a PLL? The PLL the TCD uses is pretty lame.
 ...
 // Most people who know what they are call them GPIORs
  ...
  // WHAT WERE YOU THINKING?!?!?!
  ...
  #if defined(NVMCTRL_FLBUSY_bp) && !defined(NVMCTRL_FBUSY_bp)
    #define NVMCTRL_FBUSY_bp NVMCTRL_FLBUSY_bp
  #elif !defined(NVMCTRL_FLBUSY_bp) && defined(NVMCTRL_FBUSY_bp)
    #define NVMCTRL_FLBUSY_bp NVMCTRL_FBUSY_bp
  #endif
    ...
    #define BOD_SLEEP_DISABLE_gc BOD_SLEEP_DIS_gc
    #define BOD_SLEEP_ENABLE_gc BOD_SLEEP_ENABLED_gc
    #define BOD_SLEEP_SAMPLE_gc BOD_SLEEP_SAMPLED_gc
    #define SAMPFREQ_128HZ_gc SAMPFREQ_128Hz_gc
    #define SAMPFREQ_32HZ_gc  SAMPFREQ_32Hz_gc
  ...
  #endif
  /* ======= USART ======= */
// USART_RS485_0_bm omitted. This is not an
  #if !defined(USART_RS485_0_bm) && defined(USART_RS4850_bm)
    #define USART_RXMODE_0_bm USART_RXMODE0_bm
  #elif defined(USART_RS485_0_bm) && defined(USART_RS4850_bm)
