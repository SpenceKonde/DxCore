...
// EX series starts up at 20 MHz/6 = 3.33 MHz
...
#define BAUD_SETTING_4 (((F_OSC)/6 * 64) / (16L * BAUD_RATE))
#define BAUD_ACTUAL_4  (((F_OSC)/6 * 64) / (16L * BAUD_SETTING_4))
    ...
    if (ch == STK_GET_PARAMETER) {
      ...
      getch();    // read one more byte o that we will have enough space for the trailing
        ...
        if (cmd == AVR_OP_ERASE_FLASH) {
          erase_flash(); // AVRDUDE may require a "erase_chip" command, copy from .classic part.
...
static inline void write_buffered_flash(length_t len) {
  ...
  pDst.word = address.word;
    ...
    __asm__ __volatile__ (
      ...
      "sbiw r24,2                  \n"
      "brne head                   \n"
    #ifdef __AVR_Ex__                                         //AVRxxEAxx requires write to page buffer before flash command
      "ldi r24, %[writepage]       \n" // call page write
    #else
      "clr r24                     \n" // nvmctrl noop
    #endif
...
__asm__ __volatile__(
  ...
  "eraseF:             \n"
    ...
    "add  r31,     r26 \n"    /* r26 has the erased pages byte offset */
    "brcc          .+2 \n"    /* if carry is set                      */
  ...
  ::
  [BASE]  "n" (MAPPED_PROGMEM_PAGE_SIZE >> 8),  /* start here 2nd page) */
  [CMDA]  "n" (NVMCTRL_CMD_FLPER_gc),           /* one page erase */
  [CMDB]  "n" (NVMCTRL_CMD_FLMPER32_gc + 1),    /* can't delete more then 32 pages */
  [PGCT]  "n" (MAX_ERASE_CNT)                   /* combining loop counter and increment */
  : "r24", "r25", "r26", "r31");                /*  */


#else   /* Ex only with 128 bytes in a page */
// test avr64ea28 ok
// test avr16eb28 ok
#if MAPPED_PROGMEM_PAGE_SIZE == 128
#define CDMA NVMCTRL_CMD_FLMPER4_gc
#define MAX_ERASE_CNT (CDMA + PROGMEM_SIZE/MAPPED_PROGMEM_PAGE_SIZE/32 - 1 + 3) /* e.g. 64kb: 512-4 = 4 + 8 + 16 + 15*32 */
#else // MAPPED_PROGMEM_PAGE_SIZE == 64
#define CDMA NVMCTRL_CMD_FLMPER8_gc
#define MAX_ERASE_CNT (CDMA + PROGMEM_SIZE/MAPPED_PROGMEM_PAGE_SIZE/32 - 1 + 2) /* e.g. 16kb: 256-8 = 8 + 16 + 7*32 */
#endif
  __asm__ __volatile__(        /* address low byte always 0, i.e. arbitrary */
    "ldi   r31, %[BASE] \n"    /* start at 0x200                      */
    "ldi   r26, 0x1     \n"    /* erase byte count, (pre-increment)   */
    "ldi   r27, %[CMDA] \n"    /* begin with 4/8 pages (CMDA == 0x0A) */
  "eraseF:              \n"
    "cpi   r27, %[CMDB] \n"    /* we reached max erase (CMD >= 0x0E)   */
    "brge  skip         \n"    /* don't change NVM CMD                 */
    "mov   r24, r27     \n"
  ...
  [CMDA]  "n" (CDMA),                           /* initial page erase count */
  [CMDB]  "n" (NVMCTRL_CMD_FLMPER32_gc + 1),    /* can't delete more then 32 pages */
  [PGCT]  "n" (MAX_ERASE_CNT)                   /* combining loop counter and increment */
  : "r24", "r25", "r26", "r27", "r31");
#endif
