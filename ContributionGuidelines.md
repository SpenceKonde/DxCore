...
2. If there is an existing issue that appears to be the exact same issue, read it. If your issue really does look the same, *and* the issue is either open, or was closed recently (but apparently didn't work), go ahead and comment there. By recently I mean a month or so, maybe a few months if the topic is obscure. Over 3-6 months, dollars to donuts, you've got a different bug that just has superficial similarities.
...
* Be sure to characterize the issue as completely for best results
  ...
  * Don't hesitate to include things like
    ...
    * photos of hand drawn schematic (do hesitate to post fritzing diagrams, they are very often an obstacle to successful communication); a sketch on the back of a cocktail napkin, cereal box is preferable to fritzing diagrams.
